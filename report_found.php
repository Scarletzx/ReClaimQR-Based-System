...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($item_name) || empty($category) || empty($location) || empty($description) || empty($date_found) || empty($time_found) || empty($contact_name) || empty($contact_info)) {
        $error = "Please fill in all required fields.";
    } elseif (!isset($_FILES["photo"]) || $_FILES["photo"]["error"] !== UPLOAD_ERR_OK) {
        $error = "Please upload a photo of the item.";
    } elseif (!empty($_POST['date_found']) && $_POST['date_found'] > date('Y-m-d')) {
        ...
        if (empty($error)) {
            $stmt = $conn->prepare("INSERT INTO items_found (user_id, item_name, category, location, description, date_found, time_found, contact_name, contact_info, image, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->bind_param("isssssssss", $user_id, $item_name, $category, $location, $description, $date_found, $time_found, $contact_name, $contact_info, $image_path);
            if ($stmt->execute()) {
                $success = "Found item reported successfully! It is now visible on the dashboard.";
            } else {
...
<head>
    ...
    <title>Report Found Item - ReClaimQR</title>
...
<div class="app-wrapper">
    ...
    <div class="main-content">
        ...
        <div class="page-body">
            ...
            <!-- Page Header -->
            <div class="form-page-header found-header">
            ...
            <form method="POST" action="" enctype="multipart/form-data" id="reportFoundForm">
                    ...
                    <div class="form-card">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="contact_name" placeholder="Your name"
                                   value="<?php echo htmlspecialchars($_POST['contact_name'] ?? ($_SESSION['fullname'] ?? '')); ?>"
                                   required>
                        </div>
                        <div class="form-group" style="margin-top:16px;">
                            <label>Contact <span class="required">*</span></label>
                            <input type="text" name="contact_info"
                                   placeholder="your.email@gmail.com / 01X-XXXXXX"
